...
{
	/// TODO: hay que hacer más tipos de blending... maybe
	...
	// TODO: a no se utiliza, se sustituyó por blendfactor
	this->a = 0;
}

// added by palmer -- begin

void Pixel::blendRGB(float iR, float iG, float iB, float blendFactor)
{
	/// orto tipo de blending
	
	this->r += (iR * blendFactor);
	if (this->r >255)
		this->r=255;

	this->g += (iG * blendFactor);
	if (this->g >255)
		this->g=255;
	
	this->b += (iB * blendFactor);
	if (this->b >255)
		this->b=255;
 
}

void Pixel::setToBlack () {
	this->r = 0;
	this->g = 0;
	this->b = 0;
}

void Pixel::fadeToBlack (float blendFactor) {
	ofColor color (this->r, this->g, this->b);
	color.setBrightness (color.getBrightness() * blendFactor);
	this->r = color.r;
	this->g = color.g;
	this->b = color.b;
}
